...
XWiki.Annotation = Class.create({
  ...
  initialize : function (displayHighlighted, annotatedElt, displayedByDefault) {
    ...
    if (this.displayedByDefault) {
      if (XWiki.docsyntax != 'xwiki/1.0') {
        ...
        new XWiki.widgets.Notification("Annotations are not available for pages written in XWiki/1.0 syntax.", 'warning');
      }
    }
  },
  ...
  toggleSettingsPanel : function(event) {
    ...
    if (!this.settingsPanel) {
      new Ajax.Request('http://wiki.jio.com:80/xwiki/bin/view/AnnotationCode/Settings?xpage=plain', {
        parameters : {'target' : XWiki.currentWiki + ':' + XWiki.currentSpace + '.' + XWiki.currentPage},
        onCreate: function() {
          // disable the button
          menu.disabled = true;
          // show nice loading message at page bottom
          menu._x_notification = new XWiki.widgets.Notification("Loading annotation settings", 'inprogress');
        },
  ...
  addValidateListener : function(item, id, container, inBubble) {
    item.observe('click', function(event) {
      ...
      this.updateAnnotationAsync(container, id, inBubble, item.href, 'POST',
        ...
        {
          successText : "Annotation validated.",
          failureText : "Failed:"
        });
    }.bindAsEventListener(this));
  },
  ...
  addEditListener : function(item, id, container, inBubble) {
    item.observe('click', function(event) {
      ...
      if (item.disabled) {
        ...
        require(['xwiki-meta'], function (xm) {
          new Ajax.Request('http://wiki.jio.com:80/xwiki/bin/view/AnnotationCode/EditForm', {
            parameters: {
              'xpage' : 'plain',
  ...
  fetchAndShowAnnotationDetails : function(annotationId, container) {
    require(['xwiki-meta'], function (xm) {
      new Ajax.Request('http://wiki.jio.com:80/xwiki/bin/view/AnnotationCode/DisplayForm', {
        parameters: {
          'id' : annotationId,
          'xpage' : 'plain',
          'wiki' : XWiki.currentWiki,
  ...
  onAnnotationEdit : function(event, container, annotationId, inBubble) {
    ...
    this.updateAnnotationAsync(container, annotationId, inBubble, form.action, form.method, formData,
      {
        successText : "Annotation updated.",
        failureText : "Failed:"
      });
  },
  ...
  onAddAnnotationShortcut : function() {
    ...
    if (!selectionText) {
      ...
      require(['xwiki-meta'], function (xm) {
        ...
        new Ajax.Request('http://wiki.jio.com:80/xwiki/bin/view/AnnotationCode/CreateForm', {
          parameters: {
            'xpage' : 'plain',
  ...
  onAnnotationAdd : function(event) {
    ...
    new Ajax.Request(form.action, {
      ...
      onSuccess : function (response) {
        ...
        this.fetchedAnnotations = true;
        form._x_notification = new XWiki.widgets.Notification("Annotation added.", 'done');
